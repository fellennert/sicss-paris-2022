...
We use the "old" pipe -- `%>%`. The pipe takes its argument on the left and forwards it to the next function, including it there as the first argument unless a `.` placeholder is provided somewhere. `%<>%` takes the argument on the left and changes it at the same time.
...
We also will work with lists. Lesser known functions here are the `map()` family, which applies functions over lists, and `pluck()` which extracts elements from the list. 
...
Another part of R is functions. They require arguments. Then they do something to these arguments. In the end, they return the last call (if it's not stored in an object). Otherwise, an object can be returned using `return()` -- usually unnecessary though.
