...
save(prepped_docs, sotu_stm, prep, out, file = "stm_insights.RData")
...
1. Measuring polarization of language through a "fake prediction." Download Twitter data for British and U.S.-American MPs (`timelines_gb <- read_csv("https://www.dropbox.com/s/n60co1u4ml40kvv/timelines.csv?dl=1")`, `timelines_us <- read_csv("https://www.dropbox.com/s/dpu5m3xqz4u4nv7/tweets_house_rep_party.csv?dl=1")`). Split the data into training and test set (`prop = 0.3` should suffice, make sure that you set `strata = party`). Train a model (whatever type you want, NB or RF should be reasonably fast) on it that predicts partisanship based on the Tweets' text. Predict on the test set and compare the models' accuracy. 
...
accuracy_gb <- augment(party_gb, party_tweets_test_gb)
mean(accuracy_gb$party == accuracy_gb$.pred_class)

accuracy_us <- augment(party_us, party_tweets_test_us)
mean(accuracy_us$party == accuracy_us$.pred_class)
```

2. Extract Tweets from U.S. timelines that are about abortion by using yesterday's approach (`keywords <- c("abortion", "prolife", " roe ", " wade ", "roevswade", "baby", "fetus", "womb", "prochoice", "leak")`). Perform the same prediction task (but now with `initial_split(prop = 0.8)`). How does the accuracy change with an RF model?
...
accuracy_abortion_us <- augment(abortion_us, abortion_tweets_test_us)
mean(accuracy_abortion_us$party == accuracy_abortion_us$.pred_class)
```
